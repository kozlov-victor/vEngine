...
window['showError'] = function _err(e, lineNum) {
    ...
    if (document.body) {
        ...
        popup_1.style.cssText = 'background-color:rgba(255,255,255,0.95);' + 'color:red;' + 'margin-bottom:5px;' + 'border:1px solid red;';
...
Object.defineProperty(exports, "__esModule", { value: true });
var httpClient_1 = __webpack_require__(117);
...
window.addEventListener('error', function (e) {
    ...
    try {
        ...
        httpClient_1.httpClient.get(filename, { r: Math.random() }, function (file) {
            ...
            var linesAfter = 5;
            var errorString = strings[lineNum_1 - 1] || '';
            errorString = errorString.substr(0, colNum_1 - 1) + "<span class=\"errorCol\">" + errorString[colNum_1 - 1] + "</span>" + errorString.substr(colNum_1);
            res += "<span class=\"errorRow\">" + errorString + "</span>\n";
            for (var i = 0; i < linesAfter; i++) {
                var index = lineNum_1 + i;
                var s = strings[index];
                if (s) res += s + '\n';
            }
